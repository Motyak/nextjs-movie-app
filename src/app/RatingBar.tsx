...
export default function RatingBar({value}: {value: number}) {
    ...
    let bgColorClass = percentage == 0? "" : percentage < 40 ? "bg-red-500" : percentage < 70 ? "bg-yellow-600" : "bg-green-500"
    const BAR_FULL_WIDTH_PX = 80
    let ratingBarWidthPx = Math.round(BAR_FULL_WIDTH_PX / 100 * percentage)
    if (ratingBarWidthPx < BAR_FULL_WIDTH_PX - 5 && ratingBarWidthPx > 0) {
